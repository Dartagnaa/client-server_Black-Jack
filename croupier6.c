...
int affecterPoints(char points[], int sumReduit[],int sum){
	if(sum < 10){
		points[0] = '0';
		sumReduit[0] = sum%10;
		sprintf(&points[1],"%d",sumReduit[0]);
...
void *play(void *param)
{
	...
	//envoi carte du joueur au joueur
	...
	affecterPoints(points,psumReduit,psum);
	/*if(psum < 10){
		points[0] = '0';
	...
	affecterPoints(points,psumReduit,bsum);
	envoi(j->socket,buffer,points);
