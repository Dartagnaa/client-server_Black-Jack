...
int play(joueur_ * j, int cards[])
{
	...
	{
		bcards[i]=cards[i+15]; //affecter la carte piochée au croupier
...
int main(int argc, char **argv){
    ...
    joueur_ * joueur; //liste des joueurs

	/* partie blackjack */
	int cards[52];	// Tas de carte du jeu	

    /* déclaration de la liste des joueurs */
    joueur = (joueur_ *) malloc(1 * sizeof(joueur_));

	bool rejouer = true; //refaire une partie
	...
	if ((joueur->socket = accept(socket_descriptor, (sockaddr*)(&adresse_client_courant), &longueur_adresse_courante))< 0) 
	{
	...
	do{
		...
		joueur->id = 0; //attribution de l'identifiant du joueur
		joueur->jcartes[0] = cards[1]; //attribition de la 1ère carte
		joueur->jcartes[1] = cards[6]; //attribition de la 2ème carte (décallée de 6 pour éviter que les prochains joueurs aient la même carte que les joueurs précédents)

		play(joueur,cards);
		envoi(joueur->socket, buffer, "rejouer");
		intialisationBuffer(buffer);
		sleep(5);
		if((longueur = read(socket_descriptor, buffer, sizeof(buffer))) > 0) {
			if (memcmp(&buffer[0],"n",1)==0){
				rejouer = false;
			}
		}
		sleep(2);
